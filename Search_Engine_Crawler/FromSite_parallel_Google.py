...
def search_a_site(site, port):
    ...
    lock.acquire()
    with open(f'{result_dir}/negative_data_predicted_from_site.txt', 'a', encoding='utf-8') as fp:
        for item in negative_data_predicted:
            fp.write(item)
            fp.write('\n')

    with open(f'{result_dir}/positive_data_predicted_from_site.txt', 'a', encoding='utf-8') as fp:
        for item in positive_data_predicted:
            fp.write(item)
            fp.write('\n')

    with open(f'{result_dir}/result_from_site.txt', 'a', encoding='utf-8') as fp:
    ...
    lock.release()
    # try to add some keywords for every url scheme
    keywords = ['微', '薇', '扣', 'qq', 'vx', 'tg', 'telegram',  '飞机', '@', '网', '复制', 'url']
    for i, keyword in enumerate(keywords):
        thread_pool.submit(search_a_site_with_keywords, site, keyword, 10000 + (i%15))
...
if __name__ == '__main__':
    ...
    for item in SE_link:
        ...
        if len(all_task) < max_pool:
            all_task.append(thread_pool.submit(search_a_site, item[:-1], 10000+(site_index%15)))
